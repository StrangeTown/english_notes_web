...
const highlightPart = (
	...
	if (isPlain) return full; // If plain mode, return full text without highlights
			...
			if (ptIdx !== -1) {
				...
				result.push(
					...
					>
						...
						>
							<path
								d={`M2,5 Q${pt.en.length * 2},4 ${pt.en.length * 4},5 Q${
									pt.en.length * 6
								},6 ${pt.en.length * 8},5 Q${pt.en.length * 9.2},4 ${
									pt.en.length * 9.8
								},5`}
...
function renderChineseWithPoints(item: EpisodeItem) {
	if (isPlain) return item.cn; // If plain mode, return Chinese without highlights
...
const Detail: React.FC = () => {
	...
	// Dynamically import the section data based on the param
	const [sectionData, setSectionData] = React.useState<EpisodeItem[]>([]);
	...
	useEffect(() => {
		const loadSection = async () => {
			try {
				let data;
				if (section) {
					data = await import(`../../data/finding_nemo/${section}.json`);
				} else {
					data = await import(`../../data/finding_nemo/1.json`);
				}
				setSectionData(data.default || data);
			} catch (error) {
	...
	// const practiceItems = sectionData.filter((item) => item.sentence);
